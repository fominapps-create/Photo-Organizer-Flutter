...
class GalleryScreenState extends State<GalleryScreen> {
  ...
  Future<void> _startAutoScanIfNeeded() async {
    ...
    final localUrls = imageUrls
        .where((u) => u.startsWith('local:') || u.startsWith('file:'))
        .toList();
    print('📊 Total local photos: ${localUrls.length}');
    ...
    print('🔍 Photos needing scan: ${missing.length}');
    if (missing.isEmpty) {
    ...
    print('🚀 Starting scan of ${toScan.length} photos...');
    setState(() {
      _scanning = true;
  ...
  Future<void> _scanImages(List<String> urls) async {
    print('📸 _scanImages called with ${urls.length} photos');
    for (var i = 0; i < urls.length; i++) {
      final u = urls[i];
      if (i % 10 == 0) {
        print('⏳ Scanning photo ${i + 1}/${urls.length}...');
      }
        ...
        if (file == null || !await file.exists()) {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            try {
              ...
              await TagStore.saveLocalTags(photoID, []);
            }
          } else {
  ...
  Future<void> _loadDevicePhotos() async {
    try {
      ...
      if (perm == PermissionState.authorized ||
        ...
        if (perm == PermissionState.limited) {
          print('⚠️ Limited access - loading selected photos only');
        } else {
          print('✅ Full photo access granted');
        }
      } else {
      ...
      if (albums.isEmpty) {
        print('⚠️ No albums found, trying filesystem fallback');
        // Try filesystem fallback
        try {
          final pics = await _discoverPicturesFromFs();
          print('📂 Filesystem found ${pics.length} photos');
          if (pics.isNotEmpty) {
            setState(() => imageUrls = pics);
            return;
          }
      ...
      for (final album in albums) {
        ...
        print('📸 Album "${album.name}": $count photos');
        if (count > 0) {
      ...
      print('🎯 Total unique photos: ${uniqueAssets.length}');
      ...
      print('✅ Loaded ${urls.length} photo URLs');
      ...
      print('❌ Error loading device photos: $e');
                        ...
                        children: [
                          GestureDetector(
                            ...
                            onScaleUpdate: (details) {
                              // Use threshold-based detection for better responsiveness
                              ...
                              builder: (context) {
                                var filtered = imageUrls.where((u) {
                                  ...
                                  return searchTerms.any(
                                    (searchTerm) => tags.any(
                                      (t) =>
                                          t.toLowerCase().contains(searchTerm),
                                    ),
                                  );
                                }).toList();
